...
class EffortServer(BaseServer):
    def __init__(self):
        super().__init__()
        self.server = actionlib.SimpleActionServer('effort_server', EffortAction, execute_cb= self.callback, auto_start = False)
        self.server.start()
    ...
    def callback(self, goal):
        ...
        if(self.goal.do_pitch.data): self.pub_pitch.publish(self.goal.effort.torque.y)
        if(self.goal.do_yaw.data): self.pub_yaw.publish(self.goal.effort.torque.z)
