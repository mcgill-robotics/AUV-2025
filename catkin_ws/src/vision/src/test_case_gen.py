...
def save_data():
    ...
    if pose is not None and front_cam is not None and down_cam is not None and depth_image is not None:
        title = strftime("%d_%m_%Y_%H:%M:%S")
        output_dir = test_dir + "/" + title
        os.mkdir(output_dir)
        text = open(output_dir + "/pose.txt","w")
        text.write(f"{pose.position.x},{pose.position.y},{pose.position.z},{pose.orientation.w},{pose.orientation.x},{pose.orientation.y},{pose.orientation.z}")
