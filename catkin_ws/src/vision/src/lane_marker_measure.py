...
def thresholdRed(
    ...
    ratio_img = np.uint32(img_r + max_brightness_increase) / (
        np.uint32(img_g + max_brightness_increase)
        + np.uint32(img_b + max_brightness_increase)
        + np.uint32(img_r + max_brightness_increase)
    )
    ...
    while (
        np.sum(inverted_thresh_img)
        / (255 * 3 * inverted_thresh_img.shape[0] * inverted_thresh_img.shape[1])
        < 0.2
    ):
        inverted_thresh_img = cv2.dilate(
            inverted_thresh_img, dilation_kernel, iterations=1
        )
    thresh_img = cv2.bitwise_not(inverted_thresh_img)

    if tol_publisher != None:
        tol_publisher.publish(
            bridge.cv2_to_imgmsg(thresh_img, "bgr8")
        )  # FOR ADJUSTING VALUES

    thresh_img = cv2.resize(
        thresh_img,
        dsize=(int(img.shape[1]), int(img.shape[0])),
    ...
    if blur2_amt > 0:
        thresh_img = cv2.blur(
            thresh_img,
            (
                max(2, int(blur2_amt * thresh_img.shape[0])),
                max(2, int(blur2_amt * thresh_img.shape[1])),
            ),
        )
    ...
    if thresh_publisher != None:
        thresh_publisher.publish(
            bridge.cv2_to_imgmsg(thresh_img, "mono8")
        )  # FOR ADJUSTING VALUES
...
def measure_headings(img, debug=False, debug_img=None):
    ...
    if len(lines) < 2:
        ...
        if (
            ...
            or centerPoint[0] < 0
            or centerPoint[1] < 0
            or centerPoint[0] >= thresh_img.shape[1]
            or centerPoint[1] >= thresh_img.shape[0]
        ):
            return None, None
        avgs = []
        for slope in finalLines:
            ...
            inverted_thresh_img = cv2.bitwise_not(thresh_img)
            while (
                not inverted_thresh_img[int(centerPoint[1])][int(centerPoint[0])] == 255
            ):
                inverted_thresh_img = cv2.dilate(
                    inverted_thresh_img, dilation_kernel, iterations=1
                )
