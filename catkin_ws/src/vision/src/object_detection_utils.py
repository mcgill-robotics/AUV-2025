...
class State:
    def __init__(self):
        ...
        self.camera_info_sub = rospy.Subscriber(
            "/vision/front_cam/camera_info", CameraInfo, self.updateCameraInfo
        )
    ...
    def updateX(self, msg):
        if self.paused:
            self.x_while_paused = float(msg.data)
        self.x = float(msg.data)

    def updateY(self, msg):
        if self.paused:
            self.y_while_paused = float(msg.data)
        self.y = float(msg.data)

    def updateZ(self, msg):
        if self.paused:
            self.z_while_paused = float(msg.data)
    ...
    def cleanPointCloud(self, point_cloud, bgr):
        ...
        if rospy.get_param("debug_point_cloud_cleaning"):
            ...
            ).publish(bridge.cv2_to_imgmsg(debug_img, "bgr8"))
    ...
    def getPointCloud(self, bbox=None):
        if bbox is None:
            ...
            return self.cleanPointCloud(
                np.copy(self.point_cloud), np.copy(self.bgr_img)
            )
        else:
            return self.cleanPointCloud(
                cropToBbox(self.point_cloud, bbox, copy=True),
                cropToBbox(self.bgr_img, bbox, copy=True),
            )
    ...
    def resume(self):
        if self.x_while_paused is not None:
            self.x = self.x_while_paused
        if self.y_while_paused is not None:
            self.y = self.y_while_paused
        if self.z_while_paused is not None:
            self.z = self.z_while_paused
...
def measureLaneMarker(img, bbox, debug_img):
    ...
    headings, center_point = lane_marker_measure.measure_headings(
        cropped_img, debug_img=cropToBbox(debug_img, bbox, copy=False)
    )
...
def getObjectPositionDownCam(pixel_x, pixel_y, img_height, img_width, z_pos):
    ...
    y_center_offset = (
        pixel_y - (img_height / 2)
    ) / img_height  # negated since y goes from top to bottom
...
front_cam_model_filename = (
    pwd + "/models/" + rospy.get_param("front_cam_model_filename")
)

model = [YOLO(down_cam_model_filename), YOLO(front_cam_model_filename)]
