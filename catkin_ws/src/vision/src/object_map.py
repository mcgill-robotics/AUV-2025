...
def addObservation(msg):
    ...
    for detectionFrame in msg.array:
        ...
        obj_i = findClosestObject(
            [detectionFrame.label, detectionFrame.x, detectionFrame.y, detectionFrame.z]
        )
        # if it does not pertain to any preexisting object add it to the map
        if obj_i == -1:
            object_map.append(
                [
                    detectionFrame.label,
                    detectionFrame.x,
                    detectionFrame.y,
                    detectionFrame.z,
                    detectionFrame.theta_z,
                    detectionFrame.extra_field,
                    1,
                ]
            )
        else:
...
def updateMap(obj_i, observation):
    ...
    if (
        ...
        # if there was no previous theta z but observation has a theta z set theta z to observation theta z
            ...
            if angleDifference(observed_theta_z, current_theta_z) < angleDifference(
...
sameObjectRadiusPerLabel = {
    "Lane Marker": rospy.get_param("same_object_radius_lane_marker")
}  # in same units as state_x, y, z etc (only for objects which can appear more than once)

rospy.init_node("object_map")
obj_sub = rospy.Subscriber(
    "vision/viewframe_detection", VisionObjectArray, objectDetectCb
)
