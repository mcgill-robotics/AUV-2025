...
def setup():
    ...
    addHeading(
        0, 0, 0, 0, 0, 1, auv_pub.publish, (0.5, 0.5, 0.5)
    )  # 6 - World z direction
...
def updateAUVX(msg):
    ...
    auv_marker.pose.position = Point(
        float(msg.data), auv_marker.pose.position.y, auv_marker.pose.position.z
...
def updateAUVY(msg):
    global auv_marker
    auv_marker.pose.position = Point(
        auv_marker.pose.position.x, float(msg.data), auv_marker.pose.position.z
    )  # Set the desired position
    add_breadcrumb(
        auv_marker.pose.position.x,
        auv_marker.pose.position.y,
...
def updateAUVZ(msg):
    global auv_marker
    auv_marker.pose.position = Point(
        auv_marker.pose.position.x, auv_marker.pose.position.y, float(msg.data)
    )  # Set the desired position
    add_breadcrumb(
        auv_marker.pose.position.x,
        auv_marker.pose.position.y,
...
quat_setpoint_sub = rospy.Subscriber(
    "/controls/quaternion_pid/setpoint", Quaternion, setpointQuatCb
)
