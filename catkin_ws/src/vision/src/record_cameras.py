...
def image_cb(raw_img, camera_name):
    ...
    if time.time() - lastFrameTime.get(camera_name, time.time()+10) > videoSaveTime: save_images_to_video(shutdown=False)
    #save video in smaller increments (to avoid losing everything in case of crash)
    if len(imgs.get(camera_name, [])) > maxFramesPerVideo: save_images_to_video(shutdown=False)
...
def save_images_to_video(shutdown=True):
    ...
    for camera_name in list(imgs.keys()):
        ...
        if shutdown: #if shutting down save as a pickle to go faster
            ...
            #write the video to file system as a video (slower)
            height, width, colors = imgs[camera_name][0].shape
            out = cv2.VideoWriter(video_filename + ".mp4", cv2.VideoWriter_fourcc(*'mp4v'), fps[camera_name], (width, height))
...
if __name__ == '__main__':
    ...
    maxFramesPerVideo = fps*60 #one minute video increments
