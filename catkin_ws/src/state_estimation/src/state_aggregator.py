...
def print_sensor_source_warning(axis_index, sensor_index, sensor_name):
    ...
    if (
        sensor_index != 0
        and rospy.get_time() - last_warning_message_time[axis_index]
        > sensor_swap_warning_interval
    ):
...
def update_state(msg):
    if update_state_on_clock:
        ...
        if last_clock_msg_s == msg.clock.secs and last_clock_msg_ns == msg.clock.nsecs:
            return
    ...
    for sensor_index, sensor in enumerate(sensor_priorities["orientation"]):
        if sensor.get_is_active():
            ...
            quaternion = Quaternion(
                x=reading_quaternion.x,
                y=reading_quaternion.y,
                z=reading_quaternion.z,
                w=reading_quaternion.w,
            )
            euler_dvlref_dvl = transformations.euler_from_quaternion(
                [
                    reading_quaternion.x,
                    reading_quaternion.y,
                    reading_quaternion.z,
                    reading_quaternion.w,
                ]
            )
            ...
            angular_velocity = Vector3(
                reading_angular_velocity[0],
                reading_angular_velocity[1],
                reading_angular_velocity[2],
            )
            print_sensor_source_warning(
                len(axis_names) - 1, sensor_index, sensor.get_sensor_name()
            )
            break

    for axis_index in range(len(position)):
        for sensor_index, sensor in enumerate(
            sensor_priorities[axis_names[axis_index]]
        ):
            if sensor.get_is_active():
                position[axis_index] = sensor.get_reading()[axis_names[axis_index]]
                print_sensor_source_warning(
                    axis_index, sensor_index, sensor.get_sensor_name()
                )
                break
    ...
    if position[0] is not None:
        ...
        position[0] = 0  # for publishing the Pose message
    if position[1] is not None:
        pub_y.publish(position[1])
    else:
        position[1] = 0  # for publishing the Pose message
    if position[2] is not None:
        pub_z.publish(position[2])
    else:
        position[2] = 0  # for publishing the Pose message
    if quaternion is not None:
        ...
        quaternion = Quaternion(x=0, y=0, z=0, w=1)  # for publishing the Pose message

    pose = Pose(Point(x=position[0], y=position[1], z=position[2]), quaternion)
    pub_pose.publish(pose)
    broadcast_auv_pose(pose)

    if rospy.get_time() - last_error_message_time > 1 and (
        None in position or quaternion is None
    ):
        last_error_message_time = rospy.get_time()
        rospy.logerr(
            "Missing sensor data for proper state estimation! Available states: [ X: {} Y: {} Z: {} Q: {} ]".format(
                position[0] is not None,
                position[1] is not None,
                position[2] is not None,
