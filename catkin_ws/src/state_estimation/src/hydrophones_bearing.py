...
def cb_hydrophones_time_difference(msg):
    ...
    if check_four_hydrophones(msg):
        if msg.is_pinger1_active:
            ...
            bearing_vector_local = solve_bearing_vector_3d(
            ...
            bearing_vector_global = quaternion.rotate_vectors(
                ...
                np.array(
                    [
                        bearing_vector_local[0],
                        bearing_vector_local[1],
                        bearing_vector_local[2],
                    ]
                ),
            )
        ...
        if msg.is_pinger1_active:
            ...
            bearing_vector_local = solve_bearing_vector(
                measurements[0], measurements[1]
            )
            bearing_vector_global = quaternion.rotate_vectors(
                auv_rotation,
                np.array([bearing_vector_local[0], bearing_vector_local[1], 0]),
            )
            PingerBearing_msg.pinger1_bearing.x = bearing_vector_global[0]
            PingerBearing_msg.pinger1_bearing.y = bearing_vector_global[1]
...
def check_four_hydrophones(msg):
    if (
        len(msg.dt_pinger1) == 3
        and len(msg.dt_pinger2) == 3
        and len(msg.dt_pinger3) == 3
        and len(msg.dt_pinger4) == 3
    ):
        return True
    else:
        return False
