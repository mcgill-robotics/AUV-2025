...
public:
    ...
    {
        pub_imu_ = nh.advertise<sensor_msgs::Imu>("sensors/imu/data", 10);

        sub_imu_.subscribe(nh, "imu/data", 100);
        sub_free_acc_.subscribe(nh, "filter/free_acceleration", 100);

        typedef message_filters::sync_policies::ApproximateTime<
            sensor_msgs::Imu, geometry_msgs::Vector3Stamped>
            SyncPolicy;

        sync_.reset(new message_filters::Synchronizer<SyncPolicy>(SyncPolicy(10), sub_imu_, sub_free_acc_));
        sync_->setMaxIntervalDuration(ros::Duration(0.04));
        sync_->registerCallback(boost::bind(&MovellaRepublisher::callback, this, _1, _2));
        pnh.param<std::string>("frame_id", frame_override_, "");
    }
