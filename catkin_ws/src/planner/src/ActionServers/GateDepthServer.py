...
class GateDepthServer():
    ...
    def __init__(self):
        ...
        # Heave PID
        self.heave_pid_enable_pub     = rospy.Publisher('/centroid_x_pid/enable', Bool, queue_size = 1)
        self.heave_pid_setpoint_pub   = rospy.Publisher('/centroid_x_pid/setpoint', Float64, queue_size = 1)
    ...
    def execute_cb(self, goal):
        ...
        while not (self.current_stable_counts >= self.COUNTS_FOR_STABILITY):
            ...
            self._as.publish_feedback(GateDepthFeedback(stable_count_centroid_remaining = remaining_counts))

        if success:
            rospy.loginfo('%s: Succeeded' % self._action_name)
            self._as.set_succeeded(result = GateDepthResult(Point(x=0, y=0))) # This is awful and hardcoded, please do what you want with it
