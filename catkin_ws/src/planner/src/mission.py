...
def gateMission():
    ...
    with sm:
        smach.StateMachine.add(
            "find_gate",
            InPlaceSearch(control, mapping, target_class="Gate", min_objects=1),
            transitions={"success": "navigate_gate_go_through", "failure": "failure"},
        )
        smach.StateMachine.add(
            "navigate_gate_go_through",
            NavigateGate(
                control=control,
                mapping=mapping,
                state=state,
...
def buoyMission():
    ...
    with sm:
        smach.StateMachine.add(
            "find_buoy",
            InPlaceSearch(control, mapping, target_class="Buoy", min_objects=1),
            transitions={"success": "navigate_buoy", "failure": "failure"},
        )

        smach.StateMachine.add(
            "navigate_buoy",
            NavigateBuoy(control=control, mapping=mapping, state=state),
            transitions={"success": "success", "failure": "failure"},
        )
...
def tricks(t):
    ...
    with sm:
        smach.StateMachine.add(
            "trick",
            Trick(control=control, trick_type=t),
            transitions={"success": "success", "failure": "failure"},
        )
        res = sm.execute()
...
def laneMarkerMission():
    ...
    with sm:
        smach.StateMachine.add(
            "find_lane_marker",
            BreadthFirstSearch(
                control, mapping, target_class="Lane Marker", min_objects=1
            ),
            transitions={"success": "navigate_lane_marker", "failure": "failure"},
        )

        smach.StateMachine.add(
            "navigate_lane_marker",
            NavigateLaneMarker(control, mapping, state, origin_class=""),
            transitions={"success": "success", "failure": "failure"},
        )
        res = sm.execute()
...
def semiFinals():
    ...
    with sm:
        smach.StateMachine.add(
            ...
            InPlaceSearch(control, mapping, target_class="Gate", min_objects=1),
            transitions={
                "success": "navigate_gate_no_go_through",
                "failure": "failure",
            },
        )

        smach.StateMachine.add(
            "navigate_gate_no_go_through",
            NavigateGate(
                control=control,
                mapping=mapping,
                state=state,
        ...
        smach.StateMachine.add(
            "tricks",
            Trick(control=control, trick_type="yaw", num_full_spins=2),
            transitions={"success": "navigate_gate_go_through", "failure": "failure"},
        )
        ...
        smach.StateMachine.add(
            "navigate_lane_marker",
            NavigateLaneMarker(control, mapping, state, origin_class="Gate"),
            transitions={"success": "find_buoy", "failure": "failure"},
        )

        smach.StateMachine.add(
            "find_buoy",
            LinearSearch(control, mapping, target_class="Buoy", min_objects=1),
            transitions={"success": "navigate_buoy", "failure": "failure"},
        )

        smach.StateMachine.add(
            "navigate_buoy",
            NavigateBuoy(control, mapping, state),
            transitions={"success": "find_second_lane_marker", "failure": "failure"},
        )

        smach.StateMachine.add(
            "find_second_lane_marker",
            BreadthFirstSearch(
                control, mapping, target_class="Lane Marker", min_objects=2
            ),
            transitions={
                "success": "navigate_second_lane_marker",
                "failure": "failure",
            },
        )

        smach.StateMachine.add(
            "navigate_second_lane_marker",
            NavigateLaneMarker(control, mapping, state, origin_class="Buoy"),
            transitions={"success": "find_octagon", "failure": "failure"},
        )

        smach.StateMachine.add(
            "find_octagon",
            LinearSearch(control, mapping, target_class="Octagon Table", min_objects=1),
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(
            "Waiting {} seconds before starting mission...".format(
                rospy.get_param("mission_wait_time")
            )
        )
