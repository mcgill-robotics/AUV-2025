...
def generate_trajectory(d=5.0, r=2.0, num_circle_pts=50, transition_pts=50):
    ...
    depth = -1.5 #Depth value changed from -0.5 to -1.5 only for the sim.
    z_surface = np.linspace(0.0, depth, transition_pts)
    for z in z_surface:
    ...
    for x in x_forward:
        waypoints.append([x, 0, depth])  # Y=0
    ...
    circle_z = depth * np.ones_like(theta)   # Constant depth
    ...
    # Fit a smooth parametric spline (cubic)
    tck, u = splprep([x, y, z], s=1e-6)  # CHANGED: use splprep instead of make_splprep
    return tck, waypoints  # CHANGED: return tck instead of a BSpline instance
...
def main():
    ...
    try:
        Path, waypoints = generate_trajectory(d=3.0, r=1)  # d and r to be adjusted visually in the pool.
        controls.LoS(Path, 0.5) #Lookahead distance to be adjusted visually in the pool. 
