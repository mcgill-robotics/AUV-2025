...
class NavigateGate(smach.State):
    def __init__(self, control, mapping, state, target_symbol, goThrough, gate_width):
        ...
        if target_symbol not in ["Earth Symbol", "Abydos Symbol"]:
            raise ValueError(
                "Target symbol must be one of Earth Symbol or Abydos Symbol."
            )
    ...
    def execute(self, ud):
        ...
        gate_object = self.mapping.getClosestObject(
            cls="Gate", pos=(self.state.x, self.state.y)
        )
        ...
        self.control.move(
            (gate_object[1] + offset[0], gate_object[2] + offset[1], gate_object[3])
        )  # move in front of gate
        ...
        self.mapping.updateObject(gate_object)
        symbol = (
            0 if gate_object[5] is None else gate_object[5]
        )  # 1 if earth on left, 0 if abydos left

        if self.target_symbol == "Earth Symbol":
            if symbol >= 0.5:
                print("Going through left side")
                self.control.moveDeltaLocal(
                    (0, self.gate_width / 4, 0)
                )  # a quarter of gate width
            else:
