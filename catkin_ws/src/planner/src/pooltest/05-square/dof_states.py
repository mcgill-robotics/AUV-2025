...
class Rotate(smach.State):
    def __init__(self, topic_pub, topic_sub, effort=10.0, pause=0.0, angle=180):
        super().__init__(outcomes=['done'])
        self.angle = angle
        self.pause = pause
        self.effort = effort
    ...
    def execute(self, ud):
        ...
        DELTA = 2
        rate = rospy.Rate(60)
        while(abs(self.theta - start) < self.angle - DELTA):
...
class RollIMU(Rotate):
    def __init__(self,effort,pause=0.0, angle=180):
        super().__init__("roll","state_theta_z",effort,pause=pause, angle=angle)

class PitchIMU(Rotate):
    def __init__(self,effort,pause=0.0, angle=180):
        super().__init__("pitch","state_theta_y",effort,pause=pause, angle=angle)
