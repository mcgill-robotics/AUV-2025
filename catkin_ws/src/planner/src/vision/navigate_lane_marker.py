    ...
    while not (centered and scaled):
        if len(last_object_detection) > 0:
            ...
            if scaling_error > scaling_tolerance:
                ...
                print("Heaving {}".format(heaveValue))
                scaled = False
            else:
                print("Lane marker scaled!")
            ...
            if centering_error > centering_tolerance: #isn't centered
                ...
                print("Surging {}".format(surgeValue))
                print("Swaying {}".format(swayValue))
                pubx.publish(Float64(surgeValue))
                puby.publish(Float64(swayValue))
                centered = False
            else:
                print("Lane marker centered!")
    ...
    while not success:
        while abs(rotationError) > rotationTolerance:
            if len(last_object_detection) > 0:
                ...
                print("Detection! Updating setpoint to", z_setpoint, "- current state is", state_theta_z, "(relative heading is", str(rotationError) + ")")
